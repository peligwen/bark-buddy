...
static void test_state_transitions() {
    ...
    // Drop into Low
    battery_led_update_voltage(BATTERY_LOW_MV, false);
    check(battery_led_state() == BatteryLedState::Low, "at LOW threshold -> Low");

    // Drop into Critical
    battery_led_update_voltage(BATTERY_CRITICAL_MV, false);
    ...
    battery_led_update_voltage(BATTERY_CRITICAL_MV + BATTERY_HYSTERESIS_MV - 1, false);
    ...
    battery_led_update_voltage(BATTERY_LOW_MV + BATTERY_HYSTERESIS_MV + 1, false);
...
static void test_hysteresis() {
    ...
    // Enter Low
    battery_led_update_voltage(BATTERY_LOW_MV - 1, false);
    ...
    battery_led_update_voltage(BATTERY_LOW_MV + BATTERY_HYSTERESIS_MV, false);
    check(battery_led_state() == BatteryLedState::Low,
...
// ------------------------------------------------------------------ //
// Test: cutoff latches and does not recover
// ------------------------------------------------------------------ //
static void test_cutoff_latch() {
    ...
    battery_led_update_voltage(7400, true);
    ...
    battery_led_update_voltage(8000, false);
    check(battery_led_state() == BatteryLedState::Cutoff,
...
static void test_tick_cadence() {
    ...
    battery_led_update_voltage(0, true);
