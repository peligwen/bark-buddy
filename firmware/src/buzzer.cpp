...
// Uses LEDC pin-based API (3.x); BUZZER_PIN driven directly — no channel bookkeeping.
...
void buzzer_tone(uint16_t freq_hz, uint32_t duration_ms) {
    ...
    // callback completes, ensuring no stale callback can run after ledcAttach.
    if (s_stop_timer) {
    ...
    // ledcAttach reconfigures timer frequency and re-binds the GPIO in one call.
    ledcAttach(BUZZER_PIN, freq_hz, 8);
