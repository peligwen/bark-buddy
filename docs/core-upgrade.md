...
Run: `git grep -E 'ledc|Wire|Serial|analogRead|esp_timer|freertos' firmware/src/ firmware/include/`
Common breaking changes between major versions:
- LEDC: check if channel vs pin API changed (happened in 2.x → 3.x)
- Wire: check for removed slave-mode methods
- analogRead: check default attenuation (changed in 3.x)
- FreeRTOS headers: check for moved paths
